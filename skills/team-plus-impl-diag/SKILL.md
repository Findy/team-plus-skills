...
- **当期 AI 率と月次 AI 率（#7）**: 1 回目は start_date 省略・period=three_months で呼び、`daily[]`（date / total_pulls_count / ai_pulls_count）を当期の日付範囲でクライアント合算して当期のチーム AI 率（S9）を出す（窓が当期より数日広いため total_* をそのまま使わない）。2 回目は**月次アンカー**（= 末日が「実行日 - 2日」以下である最新の暦月の、2 ヶ月前の月初）を start_date に渡して period=three_months で呼び、`daily[]` を暦月で合算して月次 AI 率（S7）を出す。アンカーは実行日だけから決まり #3 のレスポンスに依存しないため、#3 を呼ばない `scope=members` でも算出でき、`scope=all` では #3 のバケット（直近 3 完結暦月）と必ず一致する（例: 実行日が 2026-09-01 なら「実行日 - 2日」= 08-30、末日が 08-30 以下の最新月は 7 月 → アンカー = 2026-05-01）。daily[] は急落時期の特定の補助にも使う
